...
export default function OrphanageDetails() {
  ...
  const [orphanageDetail, setOrphanageDetail] = useState<Orphanages>();

  const params = route.params as OrphanageDetailsRouteParams;

  useEffect(() => {
    api.get(`orphanages/${params.id}`).then((response) => {
      setOrphanageDetail(response.data);
      console.log(orphanageDetail);
    });
  }, [params.id]);
  if (!orphanageDetail) {
    return (
  ...
  function handleOpenGoogleMapsRoutes() {
    Linking.openURL(
      `https://www.google.com/maps/dir/?api=destination=${orphanageDetail?.latitude},${orphanageDetail?.longitude}`
    );
  }
        ...
        <View style={styles.scheduleContainer}>
          <View style={[styles.scheduleItem, styles.scheduleItemBlue]}>
            ...
            <Text style={[styles.scheduleText, styles.scheduleTextBlue]}>
              Segunda à Sexta {orphanageDetail.opening_hours}
            </Text>
          </View>
